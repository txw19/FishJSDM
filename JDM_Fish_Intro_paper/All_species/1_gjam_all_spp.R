...
# Traits matrix
trmat=fread("tr_mat_NesAss_Trans2.csv")
Trmat<-as.data.frame(trmat)
rownames(Trmat) <- colnames(ydat)
colnames(Trmat)<-c("Nest.associate")

# REDUCT = F in modelList overrides automatic dimension reduction.
ml <-list(FULL=T, PREDICTX = F, ng=120000,burnin=90000,typeNames=rep("PA",dim(ydat)[2]), REDUCT=F) 
ml$random <- 'Stream'


jdm1 = gjam(~ Average_width + Average_depth + Average_substrate_size + Number_of_pools + developed,
...
# Remove intercept terms for plotting betas
...
lower <- apply(betaout,2,quantile, c(0.025))
upper <- apply(betaout,2,quantile, c(0.975))

# beta.plot <- cbind(betaMeans, lower, upper)
# head(beta.plot,10)
# dim(beta.plot)


parameter.names <- c("Average Width","Average Depth", "Substrate Size","Number of Pools","Developed")
...
for(i in 1:16){
  temp <- rep(species.names[i],length(parameter.names))
  species.names.long <- c(species.names.long,temp)
}

parameter.names.L <- rep(parameter.names,16)

beta.plot <- data.frame(betaMeans, lower, upper, species.names.long, parameter.names.L)
head(beta.plot,15)

beta.plot <- data.table(beta.plot)
# Re-order factor levels in column 'variable' for plotting
col <- "parameter.names.L"
beta.plot <-beta.plot[, (col) := factor(get(col), levels = c("Average Width","Average Depth", "Substrate Size","Number of Pools",
                                                             "Developed"))]

ggplot()+ geom_pointrange(data=beta.plot,aes(x=parameter.names.L, y=betaMeans, ymax=upper, ymin=lower), size=0.05, 
                          fill="black", shape=21)+xlab('Predictor')+
  facet_wrap(~species.names.long)+  ylab('Estimated effect') +
  theme_bw()+theme(panel.grid = element_blank(), axis.title = element_text(size=10), axis.text = element_text(size=10),
  ...
  theme(axis.text.y = element_text(size=8, angle=0)) +geom_hline(yintercept=0, linetype="dashed", 
                                                                 color = "red", size=0.1) +
